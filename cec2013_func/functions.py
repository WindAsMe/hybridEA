...
class Benchmark:
    def __init__(self, dim):
        ...
        csv_file = open(path.dirname(this_path) + '/extdata/M_D' + str(dim) + '.txt')
        ...
        csv_file = open(path.dirname(this_path) + '/extdata/shift_data.txt')
        csv_data = csv.reader(csv_file, delimiter=' ')
        self.sd = []
        for row in csv_data:
    ...
    def Y(self, X, fun_num, rflag = None):
        ...
        # 8
            ...
            self.T_asy(X_rotate, X_shift, 0.5)
            Z = self.M2 @ (self.carat(d, 10) * X_shift)
            Y = -20 * np.exp(-0.2 * np.sqrt(np.sum(Z**2)/d)) - np.exp(np.sum(np.cos(2 * np.pi * Z))/d) + 20 + np.exp(1) - 700
        ...
        # 14 15    
            ...
            Z[abs(Z) <= 500] = Z[abs(Z) <= 500] * np.sin(np.sqrt(abs(Z[abs(Z) <=500])))
            Z[Z > 500] = (500 - Z[Z > 500] % 500) * np.sin(np.sqrt(500 - Z[Z > 500] % 500)) - (Z[Z > 500] - 500)**2 / (10000*d)
            Z[Z < -500] = (abs(Z[Z < -500]) % 500 - 500) * np.sin(np.sqrt(500 - abs(Z[Z < -500]) % 500)) - (Z[Z < -500] + 500)**2 / (10000*d)
